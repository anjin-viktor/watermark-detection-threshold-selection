...
def get_correlation(path, path_reference, path_origin):
    exe_params = [dct_watermark_exe, "--get_correlation", "--correlation=0", "--in=" + path, "--reference=" + path_reference, "--skip_count=" + str(dct_skip)]
    out = subprocess.run(exe_params, capture_output=True, text=True)
...
def get_avg_dct(path, path_reference, path_origin):
    exe_params = [dct_watermark_exe, "--get_average_dct", "--in=" + path, "--reference=" + path_reference, "--skip_count=" + str(dct_skip)]
    out = subprocess.run(exe_params, capture_output=True, text=True)
