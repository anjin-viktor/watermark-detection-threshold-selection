...
def detect(path, path_reference, path_origin, threshold_level=-1):
    exe_params = [eblind_exe, "--detect", "--in=" + path, "--reference=" + path_reference, "--skip_count=" + str(dct_skip)]
...
def get_correlation(path, path_reference, path_origin):
    exe_params = [eblind_exe, "--get_correlation", "--in=" + path, "--reference=" + path_reference, "--skip_count=" + str(dct_skip)]
    out = subprocess.run(exe_params, capture_output=True, text=True)
