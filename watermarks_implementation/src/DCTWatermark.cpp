...
DCTWatermark::DCTWatermark(double alpha, double detectThreshold, int skipCount, std::vector<int> reference, bool BarniVersion, Detector::Type detectorType) : Watermark(),
	m_alpha(alpha),
	m_detectThreshold(detectThreshold),
	m_skipCount(skipCount),
	m_reference(reference),
	m_BarniVersion(BarniVersion),
...
bool DCTWatermark::embed(std::shared_ptr<VideoFrame> pframe, bool value)
{
	...
	std::size_t expectedReferenceSize = blockProcessor.getBlockCount() * (BLOCK_SIZE * BLOCK_SIZE - m_skipCount);
	...
	for (std::size_t idx = 0; idx < blockCount; idx++)
	{
		...
		for (std::size_t pos = m_skipCount; pos < BLOCK_SIZE * BLOCK_SIZE; pos++)
		{
...
Watermark::DetectResult DCTWatermark::detectInternal(const std::shared_ptr<VideoFrame> pframe, double* pcorrelation, double* averageDCT)
{
	...
	if (m_detectThreshold < 0)
		return Watermark::DetectResult::FAILED;
	if(m_detectorType != Detector::LINEAR && m_detectorType != Detector::CORRELATION_COEFFICIENT)
		return Watermark::DetectResult::FAILED;
	...
	for (std::size_t idx = 0; idx < blockCount; idx++)
	{
		...
		for (std::size_t pos = m_skipCount; pos < BLOCK_SIZE * BLOCK_SIZE; pos++, extractIdx++)
		{
...
std::vector<int> DCTWatermark::createReference(const std::shared_ptr<VideoFrame> pframe, int maxValue, int skipCount)
{
	...
	std::size_t referenceSize = blocksCount * (BLOCK_SIZE * BLOCK_SIZE - skipCount);
