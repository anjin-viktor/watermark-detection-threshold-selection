...
int main(int argc, char** argv)
{
	...
	if (vm.count("gen_reference"))
	{
		...
		std::vector<int> referenceData = DCTWatermark::createReference(pframe, vm["reference_max"].as<int>(), vm["skip_count"].as<int>());
	...
	else if (vm.count("embed"))
	{
		...
		DCTWatermark watermark(vm["alpha"].as<double>(), -1, vm["skip_count"].as<int>(), referenceData, BarniVersion);
	...
	else if (vm.count("detect"))
	{
		...
		DCTWatermark watermark(-1, vm["threshold"].as<double>(), vm["skip_count"].as<int>(), referenceData, BarniVersion, detectorType);
